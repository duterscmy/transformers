...
# iterative finetune
for tmp_prune_num_layer in range(3, prune_num_layer+1, 3):

    prune_layer_idx_to_expert_idx = {}
    for prune_layer_idx in layer_idx_list_ppl_order[:tmp_prune_num_layer]:
    ...
    print("iterative training layer {}".format(tmp_prune_num_layer))
    ...
    for layer_idx, layer in enumerate(model.model.layers):
        if layer_idx == 0:
            continue
        # set prune layer number for computation
        layer.mlp.prune_layer_num = tmp_prune_num_layer
        layer.mlp.prune_layer_idxs = layer.mlp.prune_layer_order[:tmp_prune_num_layer]
        # load pre-computed route weights
    ...
    output_file = "iterative_finetune_all_score_mode_{}_layer_{}_expert{}_ml{}_lr{}".format(
        score_mode, tmp_prune_num_layer, prune_num_expert, max_length, max_lr)
    ...
    training_args = TrainingArguments(
        ...
        save_steps=100000,                         # 不保存检查点（或者设置一个非常大的值，如1000000）
        save_strategy="steps",
