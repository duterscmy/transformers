...
def calculate_kl_divergence(probs_p, probs_q):
    ...
    :return: KL散度
    """
    probs_p = F.softmax(probs_p, dim=-1)  # 将logits转化为概率分布
    probs_q = F.softmax(probs_q, dim=-1)  # 将logits转化为概率分布
    epsilon = 1e-10
...
def get_total_js_divergence(origin_layer_outputs, prune_layer_outputs):
    ...
    for o, p in zip(origin_layer_outputs, prune_layer_outputs):
        js_div = calculate_kl_divergence(o, p)  # 改为kl
...
try:
    ...
    output_df.to_excel(
        "greedy_search_layer_prune_expert{}_stratedy{}_kl_beam{}.c4.xlsx".format(num_prune_expert, prune_expert_strategy, beam_size))
