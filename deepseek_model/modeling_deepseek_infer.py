        ...
        if self.training and self.alpha > 0.0:
            ...
            if self.seq_aux:
                ...
                ce.scatter_add_(1, topk_idx_for_aux_loss, torch.ones(
                    bsz, seq_len * aux_topk, device=hidden_states.device)).div_(seq_len * aux_topk / self.n_routed_experts)
...
class DeepseekMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([DeepseekMLP(
            config, intermediate_size=config.moe_intermediate_size) for i in range(config.n_routed_experts)])
        ...
        if self.num_route_experts == 0:
            ...
            self.prune_layer_order =[ 19, 6, 22, 15, 12, 9, 26, 18, 20, 23, 11, 3, 16, 24, 8]
        elif self.score_mode == "greedy_kl":
            self.prune_layer_order = [6, 19, 15, 22, 10, 18, 12, 26, 9, 23, 14, 21, 0, 24, 11]
        elif self.score_mode == "greedy_ppl":
            self.prune_layer_order = [25, 24, 7, 20, 1, 16, 12, 19, 6, 15, 10, 9, 18, 13, 2]
        # 确定剪枝的层
